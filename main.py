...
class GridEnv(gym.Env):
    ...
    def reset(self, *, seed: int | None = None):
        ...
        while np.array_equal(self.agent_location, self.target_location):
            self.target_location = self.np_random.integers(0, self.size, size=2, dtype=np.int32)
    ...
    def step(self, action: int):
        ...
        self.agent_location = np.clip(a=self.agent_location + direction, a_min=0, a_max=self.size - 1)
